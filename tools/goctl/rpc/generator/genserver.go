...
func (s *{{.server}}Server) {{.method}} ({{if .notStream}}ctx context.Context,{{if .hasReq}} in {{.request}}{{end}}{{else}}{{if .hasReq}} in {{.request}},{{end}}stream {{.streamBody}}{{end}}) ({{if .notStream}}{{.response}},{{end}}error) {
	l := {{.packageName}}.New{{.logicName}}({{if .notStream}}ctx,{{else}}stream.Context(),{{end}}s.svcCtx)
...
func (g *Generator) GenServer(ctx DirContext, proto parser.Proto, cfg *conf.Config) error {
	if !g.multiServiceEnabled {
		return g.genSingleServiceServer(ctx, proto, cfg, proto.Service, ctx.GetLogic())
	}
	for _, service := range proto.Services {
		if err := g.genSingleServiceServer(ctx, proto, cfg, service, ctx._GetLogic(service.Name)); err != nil {
			return err
		}
	}
	return nil
...
func (g *Generator) genSingleServiceServer(
	ctx DirContext,
	proto parser.Proto,
	cfg *conf.Config,
	...
	dir := ctx.GetServer()
	logicImport := fmt.Sprintf(`"%v"`, logicDir.Package)
